...
class MemberTest < ActiveSupport::TestCase
  test "authenticate with correct password" do
    create_member(username: "auth_test", password: "mala")

    assert Member.authenticate("auth_test", "mala")
  end

  test "authenticate with incorrect password" do
    create_member(username: "auth_test2", password: "mala")
  ...
  test "public_subs includes public subscription" do
    ...
    assert_not_includes member.public_subs, non_public_subscription
  end

  test "recent_subs returns correct number of subscriptions" do
    member = create_member
    create_subscription(member: member, feed: create_feed, created_on: 1.day.ago)
    create_subscription(member: member, feed: create_feed, created_on: 3.days.ago)
    create_subscription(member: member, feed: create_feed, created_on: 2.days.ago)
    create_subscription(member: member, feed: create_feed, created_on: 4.days.ago)
