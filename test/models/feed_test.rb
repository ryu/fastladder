...
class FeedTest < ActiveSupport::TestCase
  ...
  test "detects favicon url from feed.link" do
    ...
    feed = create_feed(link: "http://example.com/")
  ...
  test "crawlable includes ok feed with crawl_status" do
    ok_feed = create_feed(subscribers_count: 1)
    CrawlStatus.create!(feed: ok_feed, status: Fastladder::Crawler::CRAWL_OK)
    ng_feed = create_feed(subscribers_count: 1)
    CrawlStatus.create!(feed: ng_feed, status: Fastladder::Crawler::CRAWL_NOW)
  ...
  test "crawlable includes ok feed with subscribers_count" do
    ok_feed = create_feed(subscribers_count: 1)
    CrawlStatus.create!(feed: ok_feed, status: Fastladder::Crawler::CRAWL_OK)
    ng_feed = create_feed(subscribers_count: 0)
    CrawlStatus.create!(feed: ng_feed, status: Fastladder::Crawler::CRAWL_OK)
    assert_includes Feed.crawlable, ok_feed
    refute_includes Feed.crawlable, ng_feed
  end

  test "crawlable includes ok feeds based on crawled_on" do
    ok_feed_1 = create_feed(subscribers_count: 1)
    CrawlStatus.create!(feed: ok_feed_1, status: Fastladder::Crawler::CRAWL_OK, crawled_on: nil)
    ok_feed_2 = create_feed(subscribers_count: 1)
    CrawlStatus.create!(feed: ok_feed_2, status: Fastladder::Crawler::CRAWL_OK, crawled_on: 31.minutes.ago)
    ng_feed = create_feed(subscribers_count: 1)
    CrawlStatus.create!(feed: ng_feed, status: Fastladder::Crawler::CRAWL_OK, crawled_on: 29.minutes.ago)
  ...
  test "calculates average rate" do
    feed = create_feed
    create_subscription(feed: feed, member_id: 1, rate: 5)
    create_subscription(feed: feed, member_id: 2, rate: 5)
    create_subscription(feed: feed, member_id: 3, rate: 3)
